...
class Library {
	...
	public static function recordsMetadata($format, $isbn) {
	    $metadata = Library::data($isbn, 'metadata.json', 'array');
	    if (!empty($metadata) && (RECORDS_FIELDS[$format] ?? false)) {
	        $_metadata = [];
	        foreach (RECORDS_FIELDS[$format] as $field) {
	            $_metadata[$field] = self::recordsField($format, $field, $metadata);
	        }
	        return $_metadata;
	    }
	    return $metadata;
	    ...
	    switch ($format) {
	        case 'KBART': {
	            switch ($field) {
	                ...
	                case "title_url": return OPENURL.'?id='.$metadata['ean'];
	...
	public static function recordsBooks($books, $context) {
	    if ($books === 'any') {
	        $books = array_keys(Library::recordsList());
	    } else if ($books === 'all') {
	        $books = array_keys(Library::recordsList($context));
	    } else if (is_string($books)) {
	...
	public static function recordsContent($books, $format, $context, $processor = null) {
	    ...
	    if ($steps) {
	        ...
	        foreach ($books as $isbn) {
	            $records = self::recordsMetadata($format, $isbn);
	            if (!empty($records)) {
	                $metadata[] = $records;
	            }
	            if ($processor) {
	                $processor->info(0, $isbn.' : ', false);
	                $processor->report(0, !empty($records) ? 'OK' : 'KO', !empty($records) ? 'OK' : 'KO');
	            }
	        }
	        $view = new View('/'.$first[0].'/'.$first[1], ['books' => $metadata]);
	        $view->setMark(false);
	        $content = $view->render();
	        $ext = $first[0];
	        switch ($ext) {
	    ...
	    $path = ($context ?? 'any').'_'.$format.'_'.date("Y-m-d").'.'.$ext;
	    return [$content, $path];
	}
	
	public static  function recordsList($context = null) {
	    $criteria = [
	        'many' => true,
	        'order' => ['desc' => 'book']
	    ];
