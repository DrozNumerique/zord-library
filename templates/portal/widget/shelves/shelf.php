...
<?php foreach ($models['shelf']['books'] as $book) { ?>
							<tr class="book sort<?php echo !$book['readable'] ? ' unreadable' : ''; ?>" data-instances="<?php echo $models['shelf']['instances'][$book['isbn']]; ?>">
								...
								<td class="t_title title">
									<a href="<?php echo $basePath.'/book/'.$book['isbn'].(isset($models['search']) ? '?search='.$models['search'] : ''); ?>" class="content"><i class="fa fa-<?php echo $book['readable'] ? 'unlock-alt' : 'lock'; ?> fa-fw" style="color: <?php echo $book['readable'] ? 'green' : '#C0392B'; ?>;"></i><?php echo Library::title($book['title'], $book['subtitle']); ?></a>
								</td>
